...
-- -----------------------------------------------------
-- Table `TripNet`.`Marking`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `TripNet`.`Marking` ;

CREATE TABLE IF NOT EXISTS `TripNet`.`Marking` (
  `Id` INT NOT NULL,
  `Create_Time` DATETIME NULL,
  `Tour_Post_Id` INT NULL,
  `Deleted` TINYINT NULL,
  `Account_Id` INT NOT NULL,
  PRIMARY KEY (`Id`),
  INDEX `fk_Marking_Account1_idx` (`Account_Id` ASC),
  CONSTRAINT `fk_Marking_Account1`
    FOREIGN KEY (`Account_Id`)
    REFERENCES `TripNet`.`Account` (`Id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
